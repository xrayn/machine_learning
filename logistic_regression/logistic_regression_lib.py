...
def gradient_descent(X, Y, theta, lossfunc, rounds=1000, alpha=1, granularity=10, lamb=1):
    ...
    for i in range(1, rounds):
        theta, cost, cost_before, costdelta = logistic_gradient_descent_round(
            X, Y, theta, alpha, lamb)
...
def logistic_descent_optimal(X, Y, theta, lamb, method="TNC"):
    ...
    """
    Result = op.minimize(fun=logistic_cost_wrap, x0=theta, args=(
        X, Y, lamb), method=method, jac=logistic_gradient_wrap)
